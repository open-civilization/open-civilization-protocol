...
**Implemented example — domestication.** `crop_cultivation` and `animal_husbandry` are discovered exclusively through repeated Experiment: a resident foraging on terrain physically suited to the activity (see RFC-0003 Domestication and Land Improvement) has a small per-tick chance of discovering the technique. There is no scripted trigger, tick threshold, or population requirement — discovery is a probability roll gated only by physical suitability (terrain × climate zone), so whether and when domestication appears, and whether a population becomes farmers or herders, is a genuine random outcome of repeated environmental interaction. A discovery "hit" is only an attempt — a second, independent roll determines whether that particular wild variant actually becomes a stable domesticate (see RFC-0003), matching the high real-world attrition rate of domestication attempts. Once established, a random crop or livestock archetype is assigned (wheat/rice/soybean/fruit/sweet_potato/corn; grazer/browser), weighted by climate zone, giving the knowledge itself a concrete botanical/zoological identity rather than being an abstract, undifferentiated "farming" skill. The knowledge deepens further through continued practice (a form of self-reinforcing Experiment) and spreads through the same Imitation, Communication, and Inheritance pathways as any other knowledge — nothing about its acquisition or transmission is special-cased. Further agricultural technology (irrigation, selective breeding, fertilizer — RFC-0003) follows the identical Experiment pattern, each gated on a real prerequisite rather than a tick count.
